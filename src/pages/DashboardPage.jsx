...
export default function DashboardPage() {
  ...
  if (loading) return <div className="flex items-center justify-center py-32 text-gray-400 text-lg"><Clock size={28} className="animate-spin mr-3" /> Memuat data...</div>
    ...
    <div className="space-y-7">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-base text-gray-400 mt-1">Ringkasan temuan & status keseluruhan</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Temuan', num: filtered.length, color: '#6366f1', icon: TrendingUp, sub: `${active.length} aktif` },
          { label: 'Overdue', num: overdue.length, color: '#ef4444', icon: AlertTriangle, sub: 'Perlu tindakan segera' },
          { label: 'Selesai', num: archived.length, color: '#10b981', icon: CheckCircle2, sub: `${filtered.length > 0 ? Math.round((archived.length / filtered.length) * 100) : 0}% selesai` },
        ].map((s, i) => {
          const Icon = s.icon
          return (
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6 relative overflow-hidden hover:border-dark-600 transition">
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-gray-400 mt-2 uppercase tracking-wider font-semibold">Est. Biaya</p>
              <p className="text-sm text-gray-500 mt-0.5">Aktual: {formatCurrency(totalActCost)}</p>
              {withActual.length > 0 && (
                <p className={`text-sm font-bold mt-1.5 flex items-center gap-1 ${totalCostDiff > 0 ? 'text-red-400' : totalCostDiff < 0 ? 'text-emerald-400' : 'text-gray-400'}`}>
                  ...
                  Selisih: {formatCurrency(Math.abs(totalCostDiff))}
                </p>
              )}
            </div>
      ...
      {unreadNotifs > 0 && (
        <Link to="/notifications" className="flex items-center gap-4 bg-amber-500/8 border border-amber-500/15 rounded-xl px-5 py-4 hover:bg-amber-500/12 transition group">
          ...
          <p className="text-base text-amber-300/90 flex-1"><span className="font-bold text-amber-300">{unreadNotifs} notifikasi belum dibaca</span> — termasuk deadline dan status update.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6 lg:col-span-2">
          <h3 className="text-lg font-bold text-gray-100 mb-5">Temuan per Kategori</h3>
                ...
                <div key={t.value} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="text-sm text-gray-400">{t.count} total ({t.activeCount} aktif)</span>
                    </div>
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6">
          <h3 className="text-lg font-bold text-gray-100 mb-5 flex items-center gap-2"><MapPin size={20} className="text-indigo-400" /> Area Terdampak</h3>
          {areas.length === 0 ? (
            <p className="text-base text-gray-500">Tidak ada area dalam rentang ini.</p>
          ) : (
                ...
                return (
                  <div key={area} className="flex items-center justify-between px-4 py-3 bg-dark-900 border border-dark-700 rounded-xl">
                    ...
                    <div className="flex items-center gap-2">
                      {activeCount > 0 && <span className="text-xs font-bold text-amber-400 bg-amber-500/15 px-2 py-1 rounded-lg">{activeCount} aktif</span>}
          ...
          <p className="text-sm text-gray-500 mt-4 flex items-center gap-1.5"><Users size={16} /> Semua area dapat melihat seluruh temuan</p>
        </div>
      </div>
      ...
      {overdue.length > 0 && (
        <div className="bg-red-500/5 border border-red-500/15 rounded-2xl p-6">
          <h3 className="text-lg font-bold text-red-300 mb-4 flex items-center gap-2"><AlertTriangle size={20} /> Temuan Overdue</h3>
          <div className="space-y-3">
            {overdue.slice(0, 5).map(f => {
              ...
              return (
                <Link key={f.id} to={`/findings/${f.id}`} className="flex items-center gap-4 px-4 py-3.5 bg-dark-900/80 border border-dark-700 rounded-xl hover:border-red-500/30 transition group">
                  ...
                  <span className="text-sm text-red-400 font-bold shrink-0">{days} hari overdue</span>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20 text-gray-500">
          ...
          <p className="text-lg">Tidak ada temuan pada rentang tanggal ini.</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { to: '/findings', label: 'Lihat Semua Temuan', icon: '📋' },
          { to: '/archive', label: 'Arsip Selesai', icon: '📦' },
          { to: '/costs', label: 'Laporan Biaya', icon: '💰' },
          { to: '/guidelines', label: 'Guideline & Rekomendasi', icon: '📖' },
        ].map(link => (
